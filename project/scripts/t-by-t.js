...
function displayTrips(tripsToDisplay) {
    ...
    tripsToDisplay.forEach(temple => {
        ...
        people.textContent = `Travelers: ${trip.people}`;


        const budget = document.createElement("p");
        budget.textContent = `Budget: ${trip.budget}`;

        const duration = document.createElement("p");
        duration.textContent = `Duration: ${trip.duration}`;

        const transport = document.createElement("p");
        transport.textContent = `Duration: ${trip.transport}`;

        const purpose = document.createElement("p");
        purpose.textContent = `Duration: ${trip.purpose}`;


        const img = document.createElement("img");
        img.src = trip.imageUrl;
        img.alt = `Photo of ${trip.loaction}`;
        img.loading = "lazy";
        img.width = 400;
        img.height = 250;


        card.append(loaction, date, people, budget, duration, transport, purpose, img);
