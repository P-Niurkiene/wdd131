...
date.textContent = `Date: ${new Date(trip.date).toLocaleDateString("en-GB", {
    year: "numeric",
    month: "long"
})}`;
...
sortNewMenu.addEventListener("change", () => {
    let sortedTrips = [...newTrips];

    if (sortNewMenu.value === "newest") {
        sortedTrips.sort((a, b) => new Date(b.date) - new Date(a.date));
    }

    if (sortNewMenu.value === "oldest") {
        sortedTrips.sort((a, b) => new Date(a.date) - new Date(b.date));
    }

    if (sortNewMenu.value === "alpha") {
        sortedTrips.sort((a, b) => a.location.localeCompare(b.location));
    }
